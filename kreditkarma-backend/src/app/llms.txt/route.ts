...
Layered on top of your underwriting, not a replacement: underwriting judges structural risk before signing; monitoring reports OBSERVED behavioral changes in public XRP Ledger data after — once a day, for wallets you choose. Not real-time. We cannot see borrower credit, collateral, legal structure or anything off the ledger; it gives no recommendation and no probability of default; "no change observed" is not "safe".
- GET ${origin}/api/monitor — capabilities, limits, capacity statement, acknowledgement text (no key).
- POST ${origin}/api/monitor/subscribe — { addresses[≤25], webhookUrl (https), scoreDropPoints?, acknowledgement:{accepted:true,version} }. Free 3 wallets, starter 25, growth 250, scale 2,500 (per-key ceilings); the platform as a whole is capped at what the one daily monitoring run (06:00 UTC) can keep current (default 500 watched wallets; free-plan keys share at most 40% of that, so paid plans always have room) and returns 503 monitoring_capacity_reached — or monitoring_free_capacity_reached for a free key when the free pool is full — when full. The consumer-use acknowledgement (no FCRA/ECOA consumer credit, insurance, employment or housing decisions) is REQUIRED and stored with a timestamp.
- Events (webhook, HMAC-signed, secret shown once; retried; auto-disabled after repeated failure; also readable via GET /api/monitor/events): score_drop (only if YOU set scoreDropPoints — there is no default threshold), sanctions_hit / sanctions_delisted (exact match vs the OFAC SDN list), first_loan_observed, loan_overdue, loan_impaired, loan_defaulted (dormant until XLS-66 activates, then automatic), monitoring_degraded (we could not observe — says nothing about the wallet).
- GET ${origin}/api/monitor/history?subject=r... — the attested time series: sparse observations (baseline / change / event / weekly heartbeat) under the frozen monitor-observation-v1 canon, Merkle-anchored on the XRPL. A score appears in an observation only if freshly computed; an unchanged wallet carries null + a pointer. Verify: /api/attest/verify?queryId={observationId}.
