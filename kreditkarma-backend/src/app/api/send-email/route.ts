...
export async function POST(req: NextRequest) {
  try {
    ...
    const html = type === 'purchase'
      ...
      : `<h2>Grant Application Received</h2><p>Hi ${name || 'there'}, your $${amount} grant application has been received. Our team reviews every application (AI assists with triage); a human makes the decision. Please allow 24–48 hours.</p>${wallet ? `<p>Wallet: ${wallet}</p>` : ''}`
